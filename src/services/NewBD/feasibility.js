...
export const updateFeasibilityStatus = async (requestId, data, org_code) => {
  ...
  const statusId = await feasibilityModel.addFeasibilityStatus(requestId, data, org_code);
...
export const getFeasibilityStatusHistory = async (requestId, org_code) => {
  const statuses = await feasibilityModel.fetchFeasibilityStatuses(requestId, org_code);
