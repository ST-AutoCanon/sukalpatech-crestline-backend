...
export const unassignEmployeeDept = async (employeeId, departmentId, org_code) => {
  if (!employeeId || !departmentId)
    return { success: false, message: "Missing IDs" };
  const data = await model.unassignEmployeeDepartment(employeeId, departmentId, org_code);
  return { success: true, data };
};
...
export const unassignDeptPermission = async (employeeId, departmentId, org_code) => {
  if (!employeeId || !departmentId)
    return { success: false, message: "Missing IDs" };
  const data = await model.unassignPermission(employeeId, departmentId, org_code);
  return { success: true, data };
};
